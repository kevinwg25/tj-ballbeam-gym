...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        action = dist.sample()
        # ^ instead of just choosing a random sample v
        # legal = False
        # dt, max_a, av_old = info
        # while not legal: # angular
        #     action = dist.sample()  # sutck in this loop, limit sample to sample from bounded angle range
        #     theta = action.item()
        #     theta_old = state[0]
        #     v = (theta - theta_old)/dt
        #     a = (v - av_old)/dt
        #     legal = a<max_a
